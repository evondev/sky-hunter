...
- **✊ NẮM TAY = bật REFLECT LASER 5 giây** (skill kiểu Giga Wing) — xem mục riêng bên dưới.
...
- **Chế độ chuột (test)**: di chuột để lái, **giữ Space hoặc giữ chuột = bật skill** (khi charge đầy).

### ✊ Reflect Laser — skill đặc biệt 5 giây (linh hồn Giga Wing)
- Thanh charge nạp đầy (ăn năng lượng `+REFLECT.orbGain` + hồi thụ động `REFLECT.regen`/giây). Khi **ĐẦY** → **nắm tay** (hoặc giữ Space ở chế độ chuột) để kích hoạt.
...
4. **Cày lính liên tục ~3 phút** (`STAGE_DURATION = 180s`, độ khó tăng dần) → hết giờ + sạch lính thì **TRÙM khổng lồ** xuất hiện. HUD có đồng hồ **"BOSS SAU m:ss"** đếm ngược.
...
|------|-----|----------|----------------|
| `basic` | 1 | Máy bay chữ thập thường | 1 viên |
| `shooter` | 2 | Bắn đạn ngắm vào người chơi | 2 viên |
| `tank` | 5 | To, chậm, trâu máu | 3 viên |
| `boss` | 1800 + level×700 | Boss thường màn 1–6 (SIÊU TRÂU), 4 pattern bullet hell | mưa 6 viên khi chết |
| `boss cuối` | core 9000 + 2 pháo ×900 | Màn 7, **3 phase + phá bộ phận** | mưa orb khi chết |
...
- **Năng lượng dồi dào**: MỌI loại địch đều rớt năng lượng (basic 1 / shooter 2 / tank 3 viên). Mỗi viên **+2 năng lượng**.
...
- **Sprite**: `SPRITES` manifest, `loadSprite()`, `sprite(key)`, `drawSpriteImg()`; mỗi hàm vẽ (`drawShip/drawDrone/drawEnemy/drawBoss/drawFinalBoss`) ưu tiên ảnh, thiếu thì vẽ code. Ảnh ở `assets/` (xem `assets/README.md`).
...
- **Cân bằng skill laser**: object `REFLECT` — `duration` (5s), `volley`/`fireInterval` (độ dày mưa đạn), `regen`/`orbGain` (tốc nạp charge), `comboPer` (điểm combo).
- **Độ dài mỗi màn (cày bao lâu mới tới boss)**: hằng `STAGE_DURATION` (180s ≈ 3 phút). Giảm cho nhanh, tăng cho lâu.
- **Độ trâu của boss**: máu trong `spawnBoss()` (`1800 + lv*700`) và `spawnFinalBoss()` (core 9000, pháo 900). Giảm nếu thấy đánh quá lâu.
...
- **Mật độ bullet hell / FPS**: hằng `EB_CAP` (trần số đạn địch).
